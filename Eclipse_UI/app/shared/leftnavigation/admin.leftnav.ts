...
export class AdminLeftNavComponent {
    ...
    constructor(private _preferenceHelper: PreferenceHelper) {
        ...
        let role = sessionHelper.getUser().role;
        // let roleType = sessionHelper.get<IRole>('role').roleType;
        this.uiprivilege.isFirmAdmin = (role.roleTypeId == UserType.FirmAdmin);
        this.uiprivilege.isTeamAdmin = (role.roleTypeId == UserType.TeamAdmin);
        this.uiprivilege.isUser = (role.roleTypeId == UserType.User);
        ...
        this.redirectToThisRouterLink = this._preferenceHelper.redirectPrefMenuBasedOnUserRoleAndPriv(role.roleType, sessionHelper, this.eclipseAdminPrefRoute)
    }
}
