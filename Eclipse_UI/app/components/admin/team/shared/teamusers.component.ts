...
export class TeamUsersComponent {
    ...
    private createColumnDefs(resetGrid: boolean = false) {
        this.columnDefs = [
            ...
            <ColDef>{ headerName: "Added On", field: "createdOn", width: 180, cellClass: 'text-center', cellRenderer: Util.dateRenderer }
        ];
        if (this.action != VIEWV)
            this.columnDefs.push(<ColDef>{ headerName: "", field: "isDeleted", width: 110, cellRenderer: this.optionRenderer, cellClass: 'text-center' });
