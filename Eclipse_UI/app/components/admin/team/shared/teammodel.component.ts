...
export class TeamModelsComponent {
    ...
    private createColumnDefs(resetGrid: boolean = false) {
        this.columnDefs = [
            ...
            <ColDef>{ headerName: "Added On", field: "createdOn", width: 180, cellRenderer: Util.dateRenderer, cellClass: 'text-center' }
        ];
        if (this.action != VIEWV)
            this.columnDefs.push(<ColDef>{ headerName: "", field: "isDeleted", width: 80, cellRenderer: this.deleteCellRenderer, cellClass: 'text-center' });
        if (resetGrid)
