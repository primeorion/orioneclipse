...
export class CustodianPreferences extends BaseComponent {
    ...
    getCustodianDetailsByIds(custodianIds: any[]) {
        ...
        custodianIds.forEach(element => {
            apiArray.push(this._custodianService.getCustodianById(element)
        ...
        Observable.forkJoin(apiArray)
            .subscribe(custodianDetails => {
                let cnt = 0;
                custodianIds.forEach(element => {
                    this.custodian = <ICustodian>custodianDetails[cnt];
                    this.custodiansList.push(this.custodian);
                    cnt++;
                });
    ...
    bindPreferencesDataByCustId(custodianPreference: string, custodianId: number) {
        this.preferencesbo.getPreferencesData(custodianPreference, custodianId).subscribe(data => {
