...
RoleService.prototype.getRoleDetail = function (data, cb) {
	...
	roleDao.getDetail(data, function (err, fetched) {
		...
		if (fetched && fetched.length > 0) {
			...
			return cb(messages.roleNotFoud, responseCodes.NOT_FOUND);
		}
	});
};
		...
		if (data.id) {
			roleDao.delete(data, function (err, deleted) {
				...
				if (deleted.affectedRows > 0) {
					...
					return cb(messages.roleNotFoudOrDeleted, responseCodes.NOT_FOUND);
				}
			});
...
RoleService.prototype.addPrivilege = function (data, cb) {
    //check for role data
    	roleDao.get(roleEntity, function (err, role) {
    		if (err) {
    			logger.error("Getting role (addPrivilege())" + err);
    			return cb(messages.internalServerError, responseCodes.INTERNAL_SERVER_ERROR);
    		}
    		if (role && role.length > 0) {
				// semilarlly for privilege check
