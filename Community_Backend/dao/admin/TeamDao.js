...
TeamDao.prototype.add = function (data, cb) {
    ...
    queryData[teamEntity.columns.orionFirmId] = data.orionFirmId|data.user.firmId;
...
TeamDao.prototype.update = function (data, cb) {
    ...
    if (data.orionFirmId) {
        queryDataForFirmDb[teamEntity.columns.orionFirmId] = data.orionFirmId|data.user.firmId;
    }
    if (data.externalId) {
        ...
        if(result && result.length > 0){
            ...
            connection.query(query, [queryData, data.oldId], function (err, updated) {
                ...
                result.forEach(function (user) {
                    ...
                    userTeam.push(queryData.editedDate);
                    userTeam.push(queryData.editedDate);
                    userTeam.push(queryData.editedBy);
                    userTeam.push(queryData.editedBy);
                    userTeams.push(userTeam);
                });
                ...
                insertUpdateQuery.push(' AND '+userTeamEntity.columns.editedBy+' = ' + queryData.editedBy);
                insertUpdateQuery.push(' AND '+userTeamEntity.columns.editedDate+' = ' + queryData.editedDate);
                insertUpdateQuery.push(' AND '+userTeamEntity.columns.createdBy+' = ' + queryData.editedBy);
                insertUpdateQuery.push(' AND '+userTeamEntity.columns.editedDate+' = "' + queryData.editedDate);
...
TeamDao.prototype.getAdvisorPortfolios = function(data, cb){
    ...
    .where(
        squel.expr().and(squelUtils.in(accountEntity.columns.advisorId,squel.select()
    ...
    connection.query(query,[data.advisorIds], function(err, resultSet){
        if(err){
            return cb(err);
        }
        return cb(null, resultSet);
