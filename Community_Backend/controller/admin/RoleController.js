...
app.post('/', validate({ body: createOrUpdateRoleSchema }), function (req, res) {
...
app.put('/:id', validate( {params : roleIdSchema} ), validate({ body: createOrUpdateRoleSchema }), function (req, res) {
 ...
 * 
 * @apiExample Example usage:
 * curl -i http://baseurl/v1/admin/role/1
 * 
...
app.post('/:id', validate( {params : roleIdSchema} ), function (req, res) {
    ...
    var data = req.data;
    data.id = req.params.id;
    roleService.addPrivilege(req.data, function (err, status, data) {
        return response(err, status, data, res);
    });
});
