...
/**
 * Checks for the key in current session and read it.
 */
export function getFromSession<T>(key: string): T {
    let sessionHelper = new SessionHelper();
...
export function sortBy<T>(arrayList: Array<T>, fieldName: string = 'name') {
    ...
    arrayList.sort((a, b) => {
        ...
        if (typeof (a[fieldName]) == 'string' && a[fieldName].trim().toLowerCase() > b[fieldName].trim().toLowerCase()) return 1;
        if (typeof (a[fieldName]) == 'number' && a[fieldName] < b[fieldName]) return -1;
        if (typeof (a[fieldName]) == 'string' && a[fieldName].trim().toLowerCase() < b[fieldName].trim().toLowerCase()) return -1;
        if (typeof (a[fieldName]) == 'date' && a[fieldName] < b[fieldName]) return -1;
        if (typeof (a[fieldName]) == 'date' && a[fieldName] > b[fieldName]) return 1;
        return 0;
    });
    return arrayList;
 ...
 * @fieldName is field of name type T
 */
export function sortByDesc<T>(arrayList: Array<T>, fieldName: string = 'name') {
    if (isNull(arrayList)) return arrayList;
    arrayList.sort((a, b) => {
        if (typeof (a[fieldName]) == 'number' && a[fieldName] > b[fieldName]) return -1;
...
export function timeRenderer(params) {
    if (params.value == "0000-00-00 00:00:00")
        return '';
...
/** Currency format */
export function currencyCellRenderer(params) {
