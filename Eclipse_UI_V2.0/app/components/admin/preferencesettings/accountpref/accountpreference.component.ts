...
export class AccountPreferences extends BaseComponent {
    ...
    getAccountDetailsByIds(accountIds: any[]) {
        ...
        accountIds.forEach(element => {
            apiArray.push(this._accountService.getAcountDetail(element).map((response: Response) => <any>response.json()))
        ...
        Observable.forkJoin(apiArray)
            .subscribe((modelsData: any[]) => {
                ...
                let cnt = 0;
                accountIds.forEach(element => {
                    this.accountsList.push(modelsData[cnt]);
                    cnt++;
                });
    ...
    bindPreferencesDataByAccountId(accountPreference: string, accountId: number) {
        this.preferencesbo.getPreferencesData(accountPreference, accountId).subscribe((data: any[]) => {
