...
import { IPortfolioSimple, IPortfolioDetails } from '../../../../models/portfolio';
...
export class PortfolioPreferences extends BaseComponent {
    ...
    getPortfolioDetailsByIds(portfolioIds: any[]) {
        ...
        portfolioIds.forEach(element => {
            apiArray.push(this._portfolioService.getPortfolioById(element).map((response: Response) => <IPortfolioDetails>response.json()))
        ...
        Observable.forkJoin(apiArray)
            .subscribe((modelsData: any[]) => {
                let cnt = 0;
                portfolioIds.forEach(element => {
                    this.portfoliosList.push(modelsData[cnt]);
                    cnt++;
                });
    ...
    onPortfolioLoad(event) {
        this.ResponseToObjects<IPortfolioSimple>(this._portfolioService.getPortfolioSearch(event.query))
            .subscribe(model => {
                this.FilteredPorttfolioResults = model.filter(x => !x.isDeleted);
                console.log("Filtered Portfolios:", model.filter(x => !x.isDeleted));
    ...
    bindPreferencesDataByPortfolioId(portfolioPreference: string, portfolioId: number) {
        this.preferencesbo.getPreferencesData(portfolioPreference, portfolioId).subscribe((data: any[]) => {
    ...
    handleSelectedPortfolio(portfolioitem) {
        if (portfolioitem.name) {
            this._portfolioService.getPortfolioById(portfolioitem.id).map((response: Response) => <IPortfolioDetails>response.json())
                .subscribe(data => {
                    this.selectedPortfolio = undefined;
                    this.chkallDisable = false;
                    this.portfoliosList.push(data);
