...
export class CustodianListComponent extends BaseComponent {
    ...
    private createColumnDefs() {
        this.custodianColumnDefs = [
            <ColDef>{ headerName: "Custodian ID", field: "id", width: 130, cellClass: 'text-center', filter:'number' },
            <ColDef>{ headerName: "Custodian Name", field: "name", width: 135, filter:'text' },
            <ColDef>{ headerName: "Code", field: "code", width: 80, cellClass: 'text-center', filter:'text' },
            <ColDef>{ headerName: "Master Account Number", field: "accountNumber", width: 180, cellClass: 'text-center', filter:'number' },
            <ColDef>{ headerName: "Delete", field: "delete", width: 80, cellRenderer: this.deleteCellRenderer, cellClass: 'text-center', suppressMenu: true },
        ];
    }
    ...
    public onClick(targetElement) {
        ...
        else if (targetElement.title === "Delete") {
            this.deleteConfirm(selectedCustodianId);
        }
    ...
    showPopup(event) {
        switch (event) {
            case 'DELETE':
                ...
                break;
            case 'PREFERENCES':
                this._router.navigate(['/eclipse/admin/preferences/custodian',this.tabsModel.id || (this.tabsModel.ids ? this.tabsModel.ids.join() : '')]);
                break;
