...
import { ITabNav } from '../shared/team.tabnav.component';
...
export class TeamListComponent extends BaseComponent {
        ...
        this.ResponseToObjects<ITeam>(this._teamService.getTeamData())
            .subscribe(model => {
                ...
                this.teams.forEach(team => {
                     if (team.portfolioAccess == 0) team.numberOfPortfolios = -1;
                     if (team.modelAccess == 0) team.numberOfModels = -1;
                });
    ...
    private createColumnDefs() {
        this.columnDefs = [
            <ColDef>{ headerName: "Team ID", field: "id", width: 75, cellClass: 'text-center', filter: 'number' },
            <ColDef>{ headerName: "Team Name", field: "name", width: 125, filter: 'text' },
            <ColDef>{ headerName: "Created On", field: "createdOn", cellRenderer: this.dateRenderer, width: 125, cellClass: 'text-center', filter: 'text' },
            <ColDef>{ headerName: "# of Users", field: "numberOfUsers", width: 110, cellClass: 'text-center', filter: 'number' },
            <ColDef>{ headerName: "# of Portfolios", field: "numberOfPortfolios", cellRenderer: this.portfoliosRenderer, width: 110, cellClass: 'text-center', filter: 'number' },
            <ColDef>{ headerName: "# of Advisors", field: "numberOfAdvisors", width: 110, cellClass: 'text-center', filter: 'number' },
            <ColDef>{ headerName: "# of Models", field: "numberOfModels", cellRenderer: this.modelsRenderer, width: 110, cellClass: 'text-center', filter: 'number' },
            <ColDef>{ headerName: "Status", field: "status", width: 110, cellRenderer: this.statusRenderer, floatCell: true, cellClass: 'text-center', filterParams: { cellRenderer: this.statusFilterRenderer }, suppressMenu: true }
        ];
    }
    ...
    portfoliosRenderer(params) {
        ...
        if (team != null) {
            return (team.portfolioAccess == 0) ? '*' : team.numberOfPortfolios.toString();
        }
    }
    ...
    modelsRenderer(params) {
        let team = params.data;
        if (team != null) {
            return (team.modelAccess == 0) ? '*' : team.numberOfModels.toString();
        }
    }
    ...
    public onClick(targetElement) {
        ...
        if (targetElement.innerText === "View details") {
            ...
            if (this.tabsModel.ids != undefined && this.tabsModel.ids.length > 1)
                ...
                this._router.navigate(['/eclipse/admin/preferences/team', selectedTeamId|| (this.tabsModel.ids ? this.tabsModel.ids.join() : '')]);
        }
    ...
    showPopup(event) {
        switch (event) {
            ...
            case 'REASSIGN':
                ...
                break;
            case 'PREFERENCES':
                this._router.navigate(['/eclipse/admin/preferences/team', this.tabsModel.id || (this.tabsModel.ids ? this.tabsModel.ids.join() : '')]);
                break;
            default:
