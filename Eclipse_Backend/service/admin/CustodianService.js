...
CustodianService.prototype.updateCustodian = function (data, cb) {
...
CustodianService.prototype.getCustodianList = function (data, cb) {
    ...
    custodianDao.getList(data, function (err, entities) {
        ...
        custodianConverter.getListModelToResponse(entities,function(err,result){
            return cb(null, responseCode.SUCCESS, result);
        });
    });
};
...
CustodianService.prototype.getCustodianDetail = function (data, cb) {
    ...
    custodianDao.getDetail(data, function (err, entities) {
        if (err) {
        ...
        if (entities && entities.length > 0) {
            ...
            custodianConverter.getDetailModelToResponse(entity,function(err,result){
                return cb(null, responseCode.SUCCESS, result);
            });
        } else {
...
CustodianService.prototype.checkCustodianSecuritySymbolExistence = function (data, cb) {
    ...
    custodianDao.getCustodianSecuritySymbol(data, function (err, rs) {
        ...
        if(rs && (Array.isArray(rs) && rs.length > 1)){
        	return cb(messages.custodianSymbolAlreadyExistsForOtherSecurity, responseCode.DUPLICATE_ENTRY);
        }else if(rs && (Array.isArray(rs) && rs.length == 1)){
        	var row = rs[0];
        	if(row.securityId != data.securityId){
        		return cb(messages.custodianSymbolAlreadyExistsForOtherSecurity, responseCode.DUPLICATE_ENTRY);
        	}else{
        		return cb(null, responseCode.SUCCESS, rs);
        	}
    ...
    self.checkCustodianExistence(custodianEntityWithIds, function(err, status, rs){
    	if(err){
    		...
    		asyncFor(models, function(value, index, next){
    			...
    			self.checkCustodianSecuritySymbolExistence(value, function(err,status, rs){
    				if (err) {
						logger.error(err);
						next(err);
						return cb(err, status);
					}
    				custodianDao.createCustdianSecuritySymbol(value, function (err, rs) {
    					if (err) {
    						logger.error(err);
    						next(err);
    						return cb(err, responseCode.INTERNAL_SERVER_ERROR);
				...
				value.idsNotToDelete = doneCustodianIds;
				custodianDao.deleteCustdianSecuritySymbol(value, function (err, rs) {
					if (err) {
						logger.error(err);
						return cb(err, responseCode.INTERNAL_SERVER_ERROR);
					}
...
CustodianService.prototype.deleteCustodianSecuritySymbol = function (model, cb) {
    custodianDao.deleteCustdianSecuritySymbol(model, function (err, rs) {
        if (err) {
            logger.error(err);
            return cb(err, responseCode.INTERNAL_SERVER_ERROR);
        }
...
CustodianService.prototype.deleteCustodian = function (data, cb) {
...
CustodianService.prototype.getCustodianAccounts = function(data, cb){
