        ...
        teamDao.get(data, function (err, fetched) {
            ...
            if (fetched && fetched.length > 0) {
                ...
                teamDao.add(data, function (err, added) {
                    ...
                    if(data.user.roleTypeId === enums.roleType.TEAMADMIN){
                        var userIds = [];
                        userIds.push(data.user.userId)
                        data.userIds = userIds;
                        teamDao.assignUserToTeam(data, function (err,assigned){
                            if (err) {
                                logger.error("Assigning user to team (addTeam())" + err);
                                return cb(err, responseCodes.INTERNAL_SERVER_ERROR);
                            }
                            self.getTeamDetail(data, function (err, code, fetched) {
                                ...
                                /* var notificationInputData = {
                                    reqId: data.reqId,
                                    user: data.user,
                                    subject: "Team added successfully",
                                    body: "Team added successfully with id "+data.id,
                                    notificationCategoryType:enums.notificationCategoryType.TRADE_GENERATION
                                };
                                notificationService.createAndSendNotification(notificationInputData, function(err,result){
                                    if(err){
                                        logger.debug("error in create and send notification (addTeam())");
                                    }*/
        ...
        if (deleted.affectedRows > 0) {
            teamDao.getUserToInactive(data,function (err,userToInactive){
                ...
                self.removeAdvisorFromTeam(data,function(err,code,result){
                    ...
                    teamDao.getPortfolioPrimaryTeam(data,function(err,primaryPortfolios){
                        if(err){
                            logger.error("Get portfolio primary team (deleteTeam())" + err);
                            return cb(err, responseCodes.INTERNAL_SERVER_ERROR);
                        }
                        var primaryPortfolioIdsmissing = [];
                        if(primaryPortfolios && primaryPortfolios.length>0){
                            logger.info("Team has been asigned as primary team with some portfolios");
                            return cb(messages.teamAssignedAsPrimary,responseCodes.UNPROCESSABLE)
                        }else{
...
TeamService.prototype.updatePortfolioToTeam = function (data, cb) {
    ...
    teamDao.getPortfolioPrimaryTeam(data,function(err,primaryPortfolios){
        if(err){
            logger.error("Get portfolio primary team (updatePortfolioToTeam())" + err);
            return cb(err, responseCodes.INTERNAL_SERVER_ERROR);
        }
        var primaryPortfolioIdsmissing = [];
        if(primaryPortfolios && primaryPortfolios.length>0){
            var porfolioArrayInput = data.portfolioIds;
            primaryPortfolios.forEach(function(primaryPortfolio){
                var portfolioId = primaryPortfolio.portfolioId;
                if(porfolioArrayInput.indexOf(portfolioId) === -1){
                    primaryPortfolioIdsmissing.push(portfolioId);
                }
            });
        }
        if(primaryPortfolioIdsmissing && primaryPortfolioIdsmissing.length>0){
            logger.info("portfolio with this primary team are missing");
            return cb(messages.primaryPortfolioMissing+primaryPortfolioIdsmissing,responseCodes.BAD_REQUEST)
        }else{
...
TeamService.prototype.validModels = function(data,cb){
    ...
    if(modelIds && modelIds.length > 0 ){
        ...
        if(fetched && fetched.length > 0 && fetched[0].statusId === 1){
    ...
    if(portfolioIds && portfolioIds.length > 0 ){
        portfolioIds.forEach(function(id){
           ...
           portfolioDao.get(tempData,function(err,fetched){
            ...
            if(fetched && fetched.length > 0 && fetched[0].isDisabled === 0){
...
TeamService.prototype.getPrimaryPortfolioForTeam = function(data,cb){
    teamDao.getDetail(data, function (err, fetched) {
        if (err) {
            logger.error("getting team detail (getPrimaryPortfolioForTeam())" + err);
            return cb(err, responseCodes.INTERNAL_SERVER_ERROR);
        }
        if (fetched && fetched.length > 0) {
            teamDao.getPrimaryPortfolioForTeam(data,function(err,result){
                if(err){
                    logger.error("getting primary portfolio for team (getPrimaryPortfolioForTeam())" + err);
                    return cb(err, responseCodes.INTERNAL_SERVER_ERROR);
                }
                return cb(null,responseCodes.SUCCESS,result);
            });
        } else {
            logger.info("Team not Found (getPrimaryPortfolioForTeam())" + data.id);
            return cb(messages.teamNotFound, responseCodes.NOT_FOUND);
        }
    });
}
