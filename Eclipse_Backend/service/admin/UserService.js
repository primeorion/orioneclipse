		...
		if (err) {
			...
			if (results && results.length > 0) {
				...
				for (var i = 0; i < results.length; i++) {
					...
					if(row.isPrimaryTeam){
						primaryTeam.id = row.teamId;
						primaryTeam.name = row.teamName;
					}
				...
				loggedInUser.teamNames = teamNames;
				loggedInUser.primaryTeam = primaryTeam;
			}
...
UserService.prototype.searchUserFromConnect = function (inputData, cb) {
	...
	sharedCache.get(inputData.token, function (err, data) {
		...
		request(reqParams, function (err, connectResponse, data) {
			try {
				logger.debug("the output we got from connect"+data);
				return cb(err, connectResponse.statusCode, JSON.parse(data));
			} catch (err) {
...
UserService.prototype.saveProfile = function(data, cb){
	logger.info("save user profile call start (saveProfile())");
	var self = this;
	userDao.saveProfile(data,function(err,result){
		if (err) {
			logger.error("Error saving user profile (saveProfile())" + err);
			return cb(err, responseCode.INTERNAL_SERVER_ERROR);
		}
		self.getProfile(data, function(err,status, result){
			if(err){
				logger.debug("Error getting user profile(saveProfile())" + err);
				return cb(err,responseCode.INTERNAL_SERVER_ERROR);
			}
		return cb(null, responseCode.SUCCESS, result);
		})
...
UserService.prototype.getProfile = function(data, cb){
	logger.info("get user profile call start (getProfile())");

	userDao.getProfile(data,function(err,result){
		if (err) {
			logger.error("Error getting user profile (getProfile())" + err);
			return cb(err, responseCode.INTERNAL_SERVER_ERROR);
		}
		cb(null, responseCode.SUCCESS, result);
	});
