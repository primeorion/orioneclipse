...
/**
 * @api {get} /admin/teams?isActive{1/true} Get All Active Teams 
 * @apiName  GetAllActiveTeams
 * @apiVersion 1.0.0
 * @apiGroup Teams
 * @apiPermission appuser
 *
 * @apiDescription This API will return list of teams by status (Active or All). 
 *
...
app.get('/', function (req, res) {
    ...
    if (req.query.isActive && (req.query.isActive == 'true' || req.query.isActive == 1)) {
        data.isOnlyActive = 1;
    } 
    
    var model = teamConverter.getRequestToModel(data);
    
    logger.debug("Status final value "+(model.isActive));
...
/**
 * @api {get} /admin/teams/:id/primaryPortfolios Get Portfolio List where team is set as primary
 * @apiName  GetPortfolioListForPrimaryTeam.
 * @apiVersion 1.0.0
 * @apiGroup Teams
 * @apiPermission appuser
 *
 * @apiDescription This API gets Portfolio List where team is set as primary.
 *
 * @apiHeader {String} JWTToken The JWT auth token.
 *
 * @apiHeaderExample Header-Example:
 ...
 * @apiSuccess {Number}     id                      Portfolio Id
 * @apiSuccess {String}     name                    Portfolio name
 * @apiSuccess {Date}       createdOn               Team portfolio association creation date into application.
 * @apiSuccess {Number}     createdBy               Full Name of user who created the team portfolio association into the system.
 * @apiSuccess {Date}       editedOn                Team portfolio association edited date into application.
 * @apiSuccess {Number}     editedBy                Full Name of user who edited the team portfolio association into the system.
 * 
 * @apiSuccessExample {json} Success-Response:
 *     HTTP/1.1 200 OK
        [
 ...
 *     }
 */
app.get('/:id/primaryPortfolios', validate({ params: teamIdSchema }), function (req, res) {
    logger.info("Get team primary portfolio request received");

    var data = req.data;
    data.id = req.params.id;
    var model = teamConverter.getRequestToModel(data);
    teamService.getPrimaryPortfolioForTeam(model, function (err, status, data) {
        return response(err, status, data, res);
    });
});
