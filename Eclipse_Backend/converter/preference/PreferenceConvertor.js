    ...
    if (data.preferences instanceof Array) {
        data.preferences.forEach(function (record) {
            ...
            if (preferenceObj.componentType == 0) {
                ...
                preferenceObj.componentType = preferenceObj.componentType;
            }

            if (preferenceObj.componentName == "Textbox" && preferenceObj.inheritedValue != null && (preferenceObj.inheritedValue != null && ((preferenceObj.inheritedValue.trim()).length != 0)) && ((preferenceObj.value == null) ||
                    (preferenceObj.value != null && ((preferenceObj.value.trim()).length == 0)))) {
                preferenceObj.watermarkText = preferenceObj.inheritedValue;
            }            
            preferenceObj.selectedOptions.map(function (option) {
...
var savePreferenceEntityToModel_refactor = function(updatedPreferencesList){
...
PreferenceConvertor.prototype.securityPreferenceConvertor = function (data, securityPreferences, securityPreferencesValidator, cb) {
    ...
    try {
        castAll = casting.forDescriptors({
            ...
            redemptionDays: {
                type: securityPreferencesValidator["redemptionFeeDays"].type
            },
            redemptionFeeAmount: {
            ...
            sellPriority: {
                type: securityPreferencesValidator["sellPriority"].type
            },
            buyPriority: {
                type: securityPreferencesValidator["buyPriority"].type
            }
            
        });
    } catch (err) {
        logger.error("Error unable to find security preferece key (preference keys/names mismatch : " + err);
