...
ModelConverter.prototype.getModelListFromModelEntityList = function(entityList, commonTeams){
	var modelList = [];
	var map = {};
	entityList.forEach(function(entity){
		...
		_.assign(nJson, entity);
		nJson = _.omit(nJson, ["teamId", "teamName"]);

		if(map[entity.id]){
			
		}else{
			modelList.push(nJson);
			nJson.teams = [];
		}
		if(team.id || team.id == 0){			
			nJson.teams.push(team);
		}
	});
	modelList.forEach(function(val){
		var teams = _.unionWith(val.teams, commonTeams, function(team1, team2){
...
ModelConverter.prototype.getCompleteModelModelFromModelRequest = function(data, cb){
	...
	*/model.statusId = modelStatus["NOT_APPROVED"];
	...
	model.isSleeved = data.isSleeved;
	...
	if(model.portfolioId && !model.substitutedModelId){
		model.resetModelId = true;
	}
...
//	model.namespace = self.getNamespaceTeam(model);
	...
	model.currentStatusId = data.currentStatusId;
	...
	model.isSubstitutedForSleeve = data.isSubstitutedForSleeve || 0;
	model.tags = data.tags;
	model.tempModel = data.tempModel;
	model.fromTempModel = data.fromTempModel;
	model.forceFullUpdateOfApprovedModel = data.forceFullUpdateOfApprovedModel;
...
//	}
	...
	*/var modelTypeArrays = base.modelTypeArrays;
	...
	node.substituteOf = data.substitutedOf;
...
//			node.leftValue = left;
			...
			for(var i = 0; i < data.children.length; i++){
				...
				if(value.lowerModelTolerancePercent && utilService.isNegativePercent(value.lowerModelTolerancePercent)){
					msgToReturn = modelValidationMessages.percentCannotBeNegative;
				}
				if(value.upperModelTolerancePercent && utilService.isNegativePercent(value.upperModelTolerancePercent)){
					msgToReturn = modelValidationMessages.percentCannotBeNegative;
				}
				if(value.lowerTradeTolerancePercent && utilService.isNegativePercent(value.lowerTradeTolerancePercent)){
					msgToReturn = modelValidationMessages.percentCannotBeNegative;
				}
				if(value.upperTradeTolerancePercent && utilService.isNegativePercent(value.upperTradeTolerancePercent)){
					msgToReturn = modelValidationMessages.percentCannotBeNegative;
				}
				
				if(value.lowerModelToleranceAmount && utilService.isNegativePercent(value.lowerModelToleranceAmount)){
					msgToReturn = modelValidationMessages.amountCannotBeNegative;
				}
				if(value.upperModelToleranceAmount && utilService.isNegativePercent(value.upperModelToleranceAmount)){
					msgToReturn = modelValidationMessages.amountCannotBeNegative;
				}
			...
			for(var i = 0; i < data.children.length; i++){
				...
				*/if(value.isEdited){
				...
				if(value.isSubstituted){
					...
					base.isSubstitutedForSleeve = base.isSleeved;
			    }
			
				if(base.resetModelId){
					value.modelDetailId = null;
				}
...
ModelConverter.prototype.getModelEntityFromModelModel = function(data){
	...
	model.namespace = data.namespace;
	model.description = data.description;
	model.isSubModel = data.isSubModel ? data.isSubModel : 0;
	...
	model.isSubstitutedForPortfolio = data.isSubstitutedForPortfolio ? data.isSubstitutedForPortfolio : 0;
	model.isSubstitutedForSleeve = data.isSubstitutedForSleeve ? data.isSubstitutedForSleeve : 0;
...
//		}*/
		idsNotToRemove.push(modelElement.modelDetailId);
...
ModelConverter.prototype.otherPropertiesForModelElementsFromEntityToModel = function(value, node){
	...
	node.substitutedOf = value.substituteOf;
...
ModelConverter.prototype.modelSecuritiesResponseToDynamicModelSecuritiesQuantitiesEntityList = function(data){
	...
	data.list.forEach(function(value){		
		...
		rowList.push(value.modelDetailId);
...
ModelConverter.prototype.generalModelModelFromModelEntity = function(value, model){
	...
	model.isSubstitutedForSleeve = value.isSubstitutedForSleeve;
