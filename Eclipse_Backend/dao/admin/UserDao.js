...
UserDao.prototype.getDetail = function (data, cb) {
    ...
    .field(userEntity.columns.path,'userLogo')
    ...
    if(roleTypeId === enums.roleType.USER){
        query.where(
                squel.expr().and(squelUtils.in(userEntity.columns.id,data.user.userId))
            )
    }
    if(roleTypeId === enums.roleType.TEAMADMIN){
...
UserDao.prototype.getList = function (data, cb) {
    ...
    if (data.search) {
        ...
        query.order(userEntity.columns.firstName)
        query.order(userEntity.columns.lastName)
    }else{
     query.order(userEntity.columns.id)
    }
...
UserDao.prototype.assignUserToTeam = function (data, cb) {
    ...
    logger.debug("Before insert the audit user is"+utilService.getAuditUserId(data.user));
...
UserDao.prototype.getLoggedInUserWithUserId = function (data, cb) {
    ...
    var query = squel.select()
    .field(userEntity.columns.id,'id')
    .field(userTeamEntity.columns.isPrimary,'isPrimary')
    .field(userEntity.columns.orionConnectExternalId,'orionConnectExternalId')
    .field(roleEntity.columns.id,'roleId')
    .field(roleEntity.columns.name,'roleName')
...
UserDao.prototype.saveProfile = function(data, cb){
    var connection = baseDao.getConnection(data);
    var currentDate = utilDao.getSystemDateTime(null);

    var queryData = {};
    queryData[userEntity.columns.path] = data.url;

    var query = [];
    query.push("Update "+userEntity.tableName+" set ? ");
    query.push(" where "+userEntity.columns.id+" = ? ");

    query = query.join(" ");

    connection.query(query,[queryData,data.user.userId], function (err, resultSet) {
        if (err) {
            return cb(err);
        }
        return cb(null, resultSet);
...
UserDao.prototype.getProfile = function(data, cb){
    var connection = baseDao.getConnection(data);
    ...
    .field(userEntity.columns.lastName,"lastName")
    .field(userEntity.columns.email,"email")
    .field(userEntity.columns.path,"imagePath")
    .from(userEntity.tableName)
    .where(squel.expr()
        .and(squelUtils.eql(userEntity.columns.id,data.user.userId))
    );
    query = query.toString();
    logger.debug("Get Log in user profile Query = "+query);
    connection.query(query, function (err, result) {
        if (err) {
            return cb(err);
        }
