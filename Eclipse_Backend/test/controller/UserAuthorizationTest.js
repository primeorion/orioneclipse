...
describe("**** User Authorization Test Cases ****", function () {
    describe("**** START ****", function () {
                ...
                chai.request(baseurl)
                    ...
                    .end(function (err, res) {
                        setTimeout(function () {
                            ...
                            res.body.should.have.property('eclipse_access_token');
                            should.not.equal(res.body.eclipse_access_token, undefined);
                            should.not.equal(res.body.eclipse_access_token, null);
                            ...
                            eclipseAccessToken = res.body.eclipse_access_token;
                            cache.put('testLoginToken', eclipseAccessToken);
                            done();
                        });
                    });
            });
        ...
        describe("VALIDATE AUTH TOKEN # ", function () {
            if (eclipseAccessToken != null) {
                it("Should Validate Auth Token", function (done) {
                    jwt.verify(eclipseAccessToken, Constants.sessionsecret, function (fail, cb) {
                        if (fail) {
                            return fail;
                        }
                        else {
                            eclipseAccessToken = eclipseAccessToken;
                        }
                    })
                    done();
                });
                    ...
                    .end(function (err, res) {
                        setTimeout(function () {
                            if (res.status === 200) {
                                ...
                                loginAsToken = res.body.eclipse_access_token;
                                done();
                            }
                            else {
