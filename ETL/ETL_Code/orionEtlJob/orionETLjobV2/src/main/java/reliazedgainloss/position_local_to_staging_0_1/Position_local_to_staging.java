...
package reliazedgainloss.position_local_to_staging_0_1;
...
/**
 * Job: Position_local_to_staging Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.2.0.20160510_1709
 * @status 
 */
public class Position_local_to_staging implements TalendJob {
	...
	private final String jobName = "Position_local_to_staging";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Position_local_to_staging.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Position_local_to_staging.this,
	...
	public static class bulkOutStruct implements
		...
		final static byte[] commonByteArrayLock_RELIAZEDGAINLOSS_Position_local_to_staging = new byte[0];
		static byte[] commonByteArray_RELIAZEDGAINLOSS_Position_local_to_staging = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RELIAZEDGAINLOSS_Position_local_to_staging.length) {
					if (length < 1024
							&& commonByteArray_RELIAZEDGAINLOSS_Position_local_to_staging.length == 0) {
						commonByteArray_RELIAZEDGAINLOSS_Position_local_to_staging = new byte[1024];
					} else {
						commonByteArray_RELIAZEDGAINLOSS_Position_local_to_staging = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_RELIAZEDGAINLOSS_Position_local_to_staging,
						0, length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RELIAZEDGAINLOSS_Position_local_to_staging) {
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				java.io.File file_tMysqlOutputBulkExec_1_tMOB = new java.io.File(
						context.pathforlocal + "position_" + context.firmId
								+ "_" + context.date + ".tsv");
				...
				try {
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.localpath + context.positionpath,
								"ISO-8859-15", "\t", "\n", true, 0, 0, -1, -1,
								false);
					...
					while (fid_tFileInputDelimited_1 != null
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.PositionYTDRealizedSTGL = ParserUtils
											.parseTo_Float(temp);
								...
								row1.PositionYTDRealizedSTGL = null;
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.PositionYTDRealizedLTGL = ParserUtils
											.parseTo_Float(temp);
								...
								row1.PositionYTDRealizedLTGL = null;
							...
							row2.PositionYTDRealizedSTGL = row1.PositionYTDRealizedSTGL;

							row2.PositionYTDRealizedLTGL = row1.PositionYTDRealizedLTGL;
						...
						if (row1 != null) {
							...
							{ // start of Var scope
								...
								bulkOut_tmp.Price = row1.Price;
								bulkOut_tmp.PriceDate = TalendDate.parseDate(
										"MM/dd/yyyy HH:mm:ss", row1.PriceDate);
								bulkOut_tmp.AccountNumber = row1.AccountNumber;
								bulkOut_tmp.AccountId = row1.AccountId;
								bulkOut_tmp.MarketValue = row1.MarketValue;
								bulkOut_tmp.Quantity = row1.Quantity;
								bulkOut_tmp.PositionYTDRealizedSTGL = row1.PositionYTDRealizedSTGL;
								bulkOut_tmp.PositionYTDRealizedLTGL = row1.PositionYTDRealizedLTGL;
								bulkOut = bulkOut_tmp;
								// ###############################
							...
							if (bulkOut != null) {
								...
								if (bulkOut.Symbol != null) {
									...
									row_tMysqlOutputBulkExec_1_tMOB[2] = bulkOut.Symbol;

								}

								if (bulkOut.Price != null) {

									row_tMysqlOutputBulkExec_1_tMOB[3] = String
											.valueOf(bulkOut.Price);

								}

								if (bulkOut.PriceDate != null) {

									row_tMysqlOutputBulkExec_1_tMOB[4] = FormatterUtils
											.format_Date(bulkOut.PriceDate,
													"yyyy-MM-dd HH:mm:ss");
								...
								if (bulkOut.AccountNumber != null) {
									...
									row_tMysqlOutputBulkExec_1_tMOB[5] = bulkOut.AccountNumber;

								}

								if (bulkOut.AccountId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[6] = bulkOut.AccountId;

								}

								if (bulkOut.MarketValue != null) {

									row_tMysqlOutputBulkExec_1_tMOB[7] = String
											.valueOf(bulkOut.MarketValue);
								...
								if (bulkOut.PositionYTDRealizedSTGL != null) {

									row_tMysqlOutputBulkExec_1_tMOB[9] = String
											.valueOf(bulkOut.PositionYTDRealizedSTGL);

								}

								if (bulkOut.PositionYTDRealizedLTGL != null) {

									row_tMysqlOutputBulkExec_1_tMOB[10] = String
											.valueOf(bulkOut.PositionYTDRealizedLTGL);
						...
						if (row2 != null) {
							...
							{ // start of Var scope
								...
								copyOfout2_tmp.errorCode = row2.errorCode;
								copyOfout2_tmp.errorMessage = row2.errorMessage;
								copyOfout2_tmp.firmName = Var.firmname;
								copyOfout2_tmp.fileName = "possitionFile  "
										+ (context.localpath + context.positionpath);
								copyOfout2_tmp.component = "tinputfileDelimted";
								copyOfout2_tmp.process = "position Local to Staging";
							...
							if (copyOfout2 != null) {
								...
								if (copyOfout2.component == null) {
									pstmt_tAmazonMysqlOutput_1.setNull(5,
											java.sql.Types.VARCHAR);
								} else {
									pstmt_tAmazonMysqlOutput_1.setString(5,
											copyOfout2.component);
								}

								if (copyOfout2.process == null) {
					...
					if (!((Object) (context.localpath + context.positionpath) instanceof java.io.InputStream)) {
				...
				String tableName_tMysqlOutputBulkExec_1_tMBE = "position_"
				...
				while (rsTable_tMysqlOutputBulkExec_1_tMBE.next()) {
					...
					if (table_tMysqlOutputBulkExec_1_tMBE
							.equalsIgnoreCase("position_" + context.firmId
									+ "_" + context.date)) {
				...
				stmtCreate_tMysqlOutputBulkExec_1_tMBE
						.execute("CREATE TABLE `"
								+ tableName_tMysqlOutputBulkExec_1_tMBE
								+ "`(`OrionFirmId` INT(11)  ,`ExternalId` INT(11)  ,`Symbol` VARCHAR(255)  ,`Price` FLOAT(22,7)  ,`PriceDate` DATETIME ,`AccountNumber` VARCHAR(255)  ,`AccountId` VARCHAR(255)  ,`MarketValue` FLOAT(22,7)  ,`Quantity` FLOAT(22,7)  ,`PositionYTDRealizedSTGL` FLOAT(22,7)  ,`PositionYTDRealizedLTGL` FLOAT(22,7)  )");
				...
				String file_tMysqlOutputBulkExec_1_tMBE = context.pathforlocal
						+ "position_" + context.firmId + "_" + context.date
						+ ".tsv";
				...
				String bulkSQL_tMysqlOutputBulkExec_1_tMBE = "LOAD DATA LOCAL INFILE '"
						...
						+ lines_separator_tMysqlOutputBulkExec_1_tMBE
								.replaceAll("\\\\", "\\\\\\\\")
						+ "' (`OrionFirmId`,`ExternalId`,`Symbol`,`Price`,`PriceDate`,`AccountNumber`,`AccountId`,`MarketValue`,`Quantity`,`PositionYTDRealizedSTGL`,`PositionYTDRealizedLTGL`)";
	...
	public static void main(String[] args) {
		final Position_local_to_staging Position_local_to_stagingClass = new Position_local_to_staging();

		int exitCode = Position_local_to_stagingClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Position_local_to_staging.class
					.getClassLoader().getResourceAsStream(
							"reliazedgainloss/position_local_to_staging_0_1/contexts/"
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : Position_local_to_staging");
		}
...
/************************************************************************************************
 * 197389 characters generated by Talend Open Studio for Data Integration on the
 * October 24, 2016 5:33:51 PM IST
 ************************************************************************************************/
