...
package reliazedgainloss.stgtransactionimport_0_1;
...
/**
 * Job: StgTransactionImport Purpose: Stg Transaction Import<br>
 * Description: Stg Transaction Import <br>
 * @author user@talend.com
 * @version 6.2.0.20160510_1709
 * @status 
 */
public class StgTransactionImport implements TalendJob {
	...
	private final String jobName = "StgTransactionImport";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					StgTransactionImport.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(StgTransactionImport.this, new Object[] {
	...
	public static class bulkoutStruct implements
		...
		final static byte[] commonByteArrayLock_RELIAZEDGAINLOSS_StgTransactionImport = new byte[0];
		static byte[] commonByteArray_RELIAZEDGAINLOSS_StgTransactionImport = new byte[0];
		...
		public Float TradingCost;

		public Float getTradingCost() {
			return this.TradingCost;
		}
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RELIAZEDGAINLOSS_StgTransactionImport.length) {
					if (length < 1024
							&& commonByteArray_RELIAZEDGAINLOSS_StgTransactionImport.length == 0) {
						commonByteArray_RELIAZEDGAINLOSS_StgTransactionImport = new byte[1024];
					} else {
						commonByteArray_RELIAZEDGAINLOSS_StgTransactionImport = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_RELIAZEDGAINLOSS_StgTransactionImport,
						0, length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RELIAZEDGAINLOSS_StgTransactionImport) {
				...
				try {
					...
					length = dis.readByte();
					if (length == -1) {
						this.TradingCost = null;
					} else {
						this.TradingCost = dis.readFloat();
					}
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				if (this.TradingCost == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeFloat(this.TradingCost);
				}
		...
		public String toString() {
			...
			sb.append(",TradingCost=" + String.valueOf(TradingCost));
	...
	public static class row1Struct implements
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.TradeDate, dos);
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				java.io.File file_tMysqlOutputBulkExec_1_tMOB = new java.io.File(
						context.pathforlocal + "transaction_" + context.firmId
								+ "_" + context.date + ".tsv");
				...
				if (!whetherExist_tAmazonMysqlOutput_1) {
					...
					stmtCreate_tAmazonMysqlOutput_1
							.execute("CREATE TABLE `"
									...
									+ "`(`errorCode` VARCHAR(255)  ,`errorMessage` VARCHAR(255)  ,`firmName` VARCHAR(50)  ,`fileName` VARCHAR(2555)  ,`component` VARCHAR(255)  ,`process` VARCHAR(255)  )");
				...
				try {
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.localpath + context.transactionpath,
								"US-ASCII", "\t", "\n", false, 1, 0, -1, -1,
								false);
					...
					while (fid_tFileInputDelimited_1 != null
						...
						try {
							...
							row1.TradeDate = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							row1.Action = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.EclipseTradeOrderId = ParserUtils
											.parseTo_Integer(temp);
								...
								row1.EclipseTradeOrderId = null;

							}

							columnIndexWithD_tFileInputDelimited_1 = 12;

							row1.Status = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							row2.AccountId = row1.AccountId;

							row2.AccountNumber = row1.AccountNumber;

							row2.Symbol = row1.Symbol;

							row2.TradeDate = row1.TradeDate;
							...
							row2.Action = row1.Action;

							row2.TradingCost = row1.TradingCost;

							row2.EclipseTradeOrderId = row1.EclipseTradeOrderId;
						...
						if (row1 != null) {
							...
							{ // start of Var scope
								...
								bulkout_tmp.AccountId = row1.AccountId;
								bulkout_tmp.AccountNumber = row1.AccountNumber;
								bulkout_tmp.Symbol = row1.Symbol;
								bulkout_tmp.TradeDate = Relational
										.ISNULL(StringHandling
												.TRIM(row1.TradeDate))
										|| (row1.TradeDate.equals("")) ? null
										: TalendDate.parseDate("mm/dd/yyyy",
												row1.TradeDate);
								bulkout_tmp.Quantity = row1.Quantity;
								bulkout_tmp.Amount = row1.Amount;
								bulkout_tmp.Type = row1.Type;
								bulkout_tmp.Action = row1.Action;
								bulkout_tmp.TradingCost = row1.TradingCost;
								bulkout_tmp.EclipseTradeOrderId = row1.EclipseTradeOrderId;
								bulkout_tmp.Status = row1.Status;
								bulkout = bulkout_tmp;
								// ###############################
							...
							if (bulkout != null) {
								...
								if (bulkout.Symbol != null) {

									row_tMysqlOutputBulkExec_1_tMOB[4] = bulkout.Symbol;

								}

								if (bulkout.TradeDate != null) {

									row_tMysqlOutputBulkExec_1_tMOB[5] = FormatterUtils
											.format_Date(bulkout.TradeDate,
													"YYYY-MM-DD");

								}

								if (bulkout.Quantity != null) {

									row_tMysqlOutputBulkExec_1_tMOB[6] = String
											.valueOf(bulkout.Quantity);

								}

								if (bulkout.Amount != null) {

									row_tMysqlOutputBulkExec_1_tMOB[7] = String
											.valueOf(bulkout.Amount);

								}

								if (bulkout.Type != null) {

									row_tMysqlOutputBulkExec_1_tMOB[8] = bulkout.Type;

								}

								if (bulkout.Action != null) {

									row_tMysqlOutputBulkExec_1_tMOB[9] = bulkout.Action;

								}

								if (bulkout.TradingCost != null) {

									row_tMysqlOutputBulkExec_1_tMOB[10] = String
											.valueOf(bulkout.TradingCost);

								}

								if (bulkout.EclipseTradeOrderId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[11] = String
											.valueOf(bulkout.EclipseTradeOrderId);

								}

								if (bulkout.Status != null) {

									row_tMysqlOutputBulkExec_1_tMOB[12] = bulkout.Status;
						...
						if (row2 != null) {
							...
							{ // start of Var scope
								...
								input_rejected_count_tmp.errorCode = row2.errorCode;
								input_rejected_count_tmp.errorMessage = row2.errorMessage;
								input_rejected_count_tmp.firmName = Var.firmname;
								input_rejected_count_tmp.fileName = "TransactionFile  "
										+ (context.localpath + context.transactionpath);
								input_rejected_count_tmp.component = "tinputfileDelimted";
								input_rejected_count_tmp.process = "Transaction Local to Staging";
					...
					if (!((Object) (context.localpath + context.transactionpath) instanceof java.io.InputStream)) {
				...
				String tableName_tMysqlOutputBulkExec_1_tMBE = "transaction_"
				...
				while (rsTable_tMysqlOutputBulkExec_1_tMBE.next()) {
					...
					if (table_tMysqlOutputBulkExec_1_tMBE
							.equalsIgnoreCase("transaction_" + context.firmId
									+ "_" + context.date)) {
				...
				stmtCreate_tMysqlOutputBulkExec_1_tMBE
						.execute("CREATE TABLE `"
								+ tableName_tMysqlOutputBulkExec_1_tMBE
								+ "`(`OrionFirmId` INT(11)  ,`ExternalId` INT(11)  ,`AccountId` VARCHAR(255)  ,`AccountNumber` VARCHAR(255)  ,`Symbol` VARCHAR(255)  ,`TradeDate` DATETIME ,`Quantity` FLOAT(11,10)  ,`Amount` FLOAT(11,10)  ,`Type` VARCHAR(255)  ,`Action` VARCHAR(255)  ,`TradingCost` FLOAT(11,10)  ,`EclipseTradeOrderId` INT(0)  ,`Status` VARCHAR(255)  )");
				...
				String file_tMysqlOutputBulkExec_1_tMBE = context.pathforlocal
						+ "transaction_" + context.firmId + "_" + context.date
						+ ".tsv";
				...
				String bulkSQL_tMysqlOutputBulkExec_1_tMBE = "LOAD DATA LOCAL INFILE '"
						...
						+ lines_separator_tMysqlOutputBulkExec_1_tMBE
								.replaceAll("\\\\", "\\\\\\\\")
						+ "' (`OrionFirmId`,`ExternalId`,`AccountId`,`AccountNumber`,`Symbol`,`TradeDate`,`Quantity`,`Amount`,`Type`,`Action`,`TradingCost`,`EclipseTradeOrderId`,`Status`)";
	...
	public static void main(String[] args) {
		final StgTransactionImport StgTransactionImportClass = new StgTransactionImport();

		int exitCode = StgTransactionImportClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = StgTransactionImport.class
					.getClassLoader().getResourceAsStream(
							"reliazedgainloss/stgtransactionimport_0_1/contexts/"
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : StgTransactionImport");
		}
...
/************************************************************************************************
 * 195762 characters generated by Talend Open Studio for Data Integration on the
 * October 24, 2016 5:33:54 PM IST
 ************************************************************************************************/
