...
package reliazedgainloss.realizedgainlosslocaltostaging_0_1;
...
/**
 * Job: RealizedGainLossLocaltoStaging Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 6.2.0.20160510_1709
 * @status 
 */
public class RealizedGainLossLocaltoStaging implements TalendJob {
	...
	private final String jobName = "RealizedGainLossLocaltoStaging";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					RealizedGainLossLocaltoStaging.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(RealizedGainLossLocaltoStaging.this,
	...
	public static class mainOutStruct implements
		...
		final static byte[] commonByteArrayLock_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging = new byte[0];
		static byte[] commonByteArray_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging.length) {
					if (length < 1024
							&& commonByteArray_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging.length == 0) {
						commonByteArray_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging = new byte[1024];
					} else {
						commonByteArray_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging,
						0, length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging) {
	...
	public static class row1Struct implements
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RELIAZEDGAINLOSS_RealizedGainLossLocaltoStaging) {
				...
				try {
					...
					this.DateAquired = readString(dis);

					this.SellDate = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.DateAquired, dos);

				// String

				writeString(this.SellDate, dos);
		...
		public String toString() {
			...
			sb.append(",NetProceeds=" + String.valueOf(NetProceeds));
			sb.append(",CostAmount=" + String.valueOf(CostAmount));
			sb.append(",DateAquired=" + DateAquired);
			sb.append(",SellDate=" + SellDate);
			sb.append(",Quantity=" + String.valueOf(Quantity));
			sb.append(",LongTerm=" + String.valueOf(LongTerm));
			sb.append(",SellMethod=" + SellMethod);
			sb.append(",TotalGains=" + String.valueOf(TotalGains));
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				java.io.File file_tMysqlOutputBulkExec_1_tMOB = new java.io.File(
						context.pathforlocal + "realizedgainloss_"
								+ context.firmId + "_" + context.date + ".tsv");
				...
				try {
					...
					while (fid_tFileInputDelimited_1 != null
						...
						try {
							...
							if (temp.length() > 0) {
								...
								try {
									...
									row1.GrossProceeds = ParserUtils
											.parseTo_Double(temp);
							...
							row1.DateAquired = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);

							columnIndexWithD_tFileInputDelimited_1 = 9;

							row1.SellDate = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							row1.SellMethod = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							row2.GrossProceeds = row1.GrossProceeds;

							row2.NetProceeds = row1.NetProceeds;

							row2.CostAmount = row1.CostAmount;

							row2.DateAquired = row1.DateAquired;

							row2.SellDate = row1.SellDate;

							row2.Quantity = row1.Quantity;

							row2.LongTerm = row1.LongTerm;

							row2.SellMethod = row1.SellMethod;

							row2.TotalGains = row1.TotalGains;
						...
						if (row1 != null) {
							...
							{ // start of Var scope
								...
								mainOut_tmp.OrionFirmId = row1.OrionFirmId;
								mainOut_tmp.ExternalId = row1.ExternalId;
								mainOut_tmp.Symbol = row1.Symbol;
								mainOut_tmp.AccountId = row1.AccountId;
								mainOut_tmp.AccountNumber = row1.AccountNumber;
								mainOut_tmp.GrossProceeds = row1.GrossProceeds;
								mainOut_tmp.NetProceeds = row1.NetProceeds;
								mainOut_tmp.CostAmount = row1.CostAmount;
								mainOut_tmp.DateAquired = TalendDate
										.parseDate("yyyy-MM-dd HH:mm:ss",
												row1.DateAquired);
								mainOut_tmp.SellDate = TalendDate.parseDate(
										"yyyy-MM-dd HH:mm:ss", row1.SellDate);
								mainOut_tmp.Quantity = row1.Quantity;
								mainOut_tmp.LongTerm = row1.LongTerm;
								mainOut_tmp.SellMethod = row1.SellMethod;
								mainOut_tmp.TotalGains = row1.TotalGains;
								mainOut = mainOut_tmp;
								// ###############################
							...
							if (mainOut != null) {
								...
								if (mainOut.OrionFirmId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[0] = String
											.valueOf(mainOut.OrionFirmId);

								}

								if (mainOut.ExternalId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[1] = String
											.valueOf(mainOut.ExternalId);

								}

								if (mainOut.Symbol != null) {

									row_tMysqlOutputBulkExec_1_tMOB[2] = mainOut.Symbol;

								}

								if (mainOut.AccountId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[3] = mainOut.AccountId;

								}

								if (mainOut.AccountNumber != null) {

									row_tMysqlOutputBulkExec_1_tMOB[4] = mainOut.AccountNumber;

								}

								if (mainOut.GrossProceeds != null) {

									row_tMysqlOutputBulkExec_1_tMOB[5] = String
											.valueOf(mainOut.GrossProceeds);

								}

								if (mainOut.NetProceeds != null) {

									row_tMysqlOutputBulkExec_1_tMOB[6] = String
											.valueOf(mainOut.NetProceeds);

								}

								if (mainOut.CostAmount != null) {

									row_tMysqlOutputBulkExec_1_tMOB[7] = String
											.valueOf(mainOut.CostAmount);

								}

								if (mainOut.DateAquired != null) {

									row_tMysqlOutputBulkExec_1_tMOB[8] = FormatterUtils
											.format_Date(mainOut.DateAquired,
													"yyyy-MM-dd HH:mm:ss");

								}

								if (mainOut.SellDate != null) {

									row_tMysqlOutputBulkExec_1_tMOB[9] = FormatterUtils
											.format_Date(mainOut.SellDate,
													"yyyy-MM-dd HH:mm:ss");

								}

								if (mainOut.Quantity != null) {

									row_tMysqlOutputBulkExec_1_tMOB[10] = String
											.valueOf(mainOut.Quantity);

								}

								if (mainOut.LongTerm != null) {

									row_tMysqlOutputBulkExec_1_tMOB[11] = String
											.valueOf(true == mainOut.LongTerm ? "1"
													: "0");

								}

								if (mainOut.SellMethod != null) {

									row_tMysqlOutputBulkExec_1_tMOB[12] = mainOut.SellMethod;

								}

								if (mainOut.TotalGains != null) {

									row_tMysqlOutputBulkExec_1_tMOB[13] = String
											.valueOf(mainOut.TotalGains);
						...
						if (row2 != null) {
							...
							{ // start of Var scope
								...
								input_rejected_count_tmp.fileName = "RealizedGainLoss  "
										+ (context.localpath + context.taxlotpath);
								input_rejected_count_tmp.component = "tinputfileDelimted";
								input_rejected_count_tmp.process = "RealizedGainLoss Local to Staging";
					...
					if (!((Object) (context.localpath + context.RealizedGainLossPath) instanceof java.io.InputStream)) {
				...
				String tableName_tMysqlOutputBulkExec_1_tMBE = "realizedgainloss_"
				...
				while (rsTable_tMysqlOutputBulkExec_1_tMBE.next()) {
					...
					if (table_tMysqlOutputBulkExec_1_tMBE
							.equalsIgnoreCase("realizedgainloss_"
									+ context.firmId + "_" + context.date)) {
				...
				String file_tMysqlOutputBulkExec_1_tMBE = context.pathforlocal
						+ "realizedgainloss_" + context.firmId + "_"
						+ context.date + ".tsv";
				...
				String bulkSQL_tMysqlOutputBulkExec_1_tMBE = "LOAD DATA LOCAL INFILE '"
						...
						+ lines_separator_tMysqlOutputBulkExec_1_tMBE
								.replaceAll("\\\\", "\\\\\\\\")
						+ "' (`OrionFirmId`,`ExternalId`,`Symbol`,`AccountId`,`AccountNumber`,`GrossProceeds`,`NetProceeds`,`CostAmount`,`DateAquired`,`SellDate`,`Quantity`,@tMysqlOutputBulkExec_1_tMBE_LongTerm,`SellMethod`,`TotalGains`)"
						+ " SET"
						+ " LongTerm=CAST(@tMysqlOutputBulkExec_1_tMBE_LongTerm AS UNSIGNED)";
	...
	public static void main(String[] args) {
		final RealizedGainLossLocaltoStaging RealizedGainLossLocaltoStagingClass = new RealizedGainLossLocaltoStaging();

		int exitCode = RealizedGainLossLocaltoStagingClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = RealizedGainLossLocaltoStaging.class
					.getClassLoader().getResourceAsStream(
							"reliazedgainloss/realizedgainlosslocaltostaging_0_1/contexts/"
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : RealizedGainLossLocaltoStaging");
		}
...
/************************************************************************************************
 * 207593 characters generated by Talend Open Studio for Data Integration on the
 * October 24, 2016 5:33:58 PM IST
 ************************************************************************************************/
