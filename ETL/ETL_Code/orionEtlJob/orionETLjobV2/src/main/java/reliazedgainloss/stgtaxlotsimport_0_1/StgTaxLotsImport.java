...
package reliazedgainloss.stgtaxlotsimport_0_1;
...
/**
 * Job: StgTaxLotsImport Purpose: Stg TaxLots Import<br>
 * Description: Stg TaxLots Import <br>
 * @author user@talend.com
 * @version 6.2.0.20160510_1709
 * @status 
 */
public class StgTaxLotsImport implements TalendJob {
	...
	private final String jobName = "StgTaxLotsImport";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					StgTaxLotsImport.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(StgTaxLotsImport.this, new Object[] { e,
	...
	public static class bulkouttaxlotStruct implements
			routines.system.IPersistableRow<bulkouttaxlotStruct> {
		final static byte[] commonByteArrayLock_RELIAZEDGAINLOSS_StgTaxLotsImport = new byte[0];
		static byte[] commonByteArray_RELIAZEDGAINLOSS_StgTaxLotsImport = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_RELIAZEDGAINLOSS_StgTaxLotsImport.length) {
					if (length < 1024
							&& commonByteArray_RELIAZEDGAINLOSS_StgTaxLotsImport.length == 0) {
						commonByteArray_RELIAZEDGAINLOSS_StgTaxLotsImport = new byte[1024];
					} else {
						commonByteArray_RELIAZEDGAINLOSS_StgTaxLotsImport = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_RELIAZEDGAINLOSS_StgTaxLotsImport, 0,
						length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RELIAZEDGAINLOSS_StgTaxLotsImport) {
		 ...
		 * Compare keys
		 */
		public int compareTo(bulkouttaxlotStruct other) {
	...
	public static class row1Struct implements
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_RELIAZEDGAINLOSS_StgTaxLotsImport) {
				...
				try {
					...
					this.DateAcquired = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.DateAcquired, dos);
				...
				writeString(this.PriceDate, dos);
		...
		public String toString() {
			...
			sb.append(",Symbol=" + Symbol);
			sb.append(",AccountNumber=" + AccountNumber);
			sb.append(",DateAcquired=" + DateAcquired);
			sb.append(",Quantity=" + String.valueOf(Quantity));
			sb.append(",CostAmount=" + String.valueOf(CostAmount));
			sb.append(",CostPerShare=" + String.valueOf(CostPerShare));
			sb.append(",Price=" + String.valueOf(Price));
			sb.append(",MarketValue=" + String.valueOf(MarketValue));
			sb.append(",PriceDate=" + PriceDate);
			sb.append("]");
	...
	public void tFileInputDelimited_1Process(
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				bulkouttaxlotStruct bulkouttaxlot = new bulkouttaxlotStruct();
				...
				java.io.File file_tMysqlOutputBulkExec_1_tMOB = new java.io.File(
						context.pathforlocal + "taxLots_" + context.firmId
								+ "_" + context.date + ".tsv");
				...
				bulkouttaxlotStruct bulkouttaxlot_tmp = new bulkouttaxlotStruct();
				// ###############################
				...
				try {
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.localpath + context.taxlotpath,
								"US-ASCII", "\t", "\n", false, 1, 0, -1, -1,
								false);
					...
					while (fid_tFileInputDelimited_1 != null
						...
						try {
							...
							row1.DateAcquired = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							if (temp.length() > 0) {
								...
								row1.MarketValue = null;

							}

							columnIndexWithD_tFileInputDelimited_1 = 11;

							row1.PriceDate = fid_tFileInputDelimited_1
									.get(columnIndexWithD_tFileInputDelimited_1);
							...
							row2.DateAcquired = row1.DateAcquired;

							row2.Quantity = row1.Quantity;

							row2.CostAmount = row1.CostAmount;

							row2.CostPerShare = row1.CostPerShare;

							row2.Price = row1.Price;

							row2.MarketValue = row1.MarketValue;

							row2.PriceDate = row1.PriceDate;
						...
						if (row1 != null) {
							...
							{ // start of Var scope
								...
								// # Output table : 'bulkouttaxlot'
								bulkouttaxlot_tmp.OrionFirmId = row1.OrionFirmId;
								bulkouttaxlot_tmp.ExternalId = row1.ExternalId;
								bulkouttaxlot_tmp.AccountId = row1.AccountId;
								bulkouttaxlot_tmp.Symbol = row1.Symbol;
								bulkouttaxlot_tmp.AccountNumber = row1.AccountNumber;
								bulkouttaxlot_tmp.DateAcquired = Relational
										.ISNULL(StringHandling
												.TRIM(row1.DateAcquired))
										|| (row1.DateAcquired.equals("")) ? null
										: TalendDate.parseDate("mm/dd/yyyy",
												row1.DateAcquired);
								bulkouttaxlot_tmp.Quantity = row1.Quantity;
								bulkouttaxlot_tmp.CostAmount = row1.CostAmount;
								bulkouttaxlot_tmp.CostPerShare = row1.CostPerShare;
								bulkouttaxlot_tmp.Price = row1.Price;
								bulkouttaxlot_tmp.MarketValue = row1.MarketValue;
								bulkouttaxlot_tmp.PriceDate = Relational
										.ISNULL(StringHandling
												.TRIM(row1.PriceDate))
										|| (row1.PriceDate.equals("")) ? null
										: TalendDate.parseDate(
												"MM/DD/yyyy HH:mm:ss",
												row1.PriceDate);
								bulkouttaxlot_tmp.id = Numeric.sequence("s4",
										1, 1);
								bulkouttaxlot = bulkouttaxlot_tmp;
								// ###############################
							 ...
							 * [tMap_1 main ] stop
							 */
							// Start of branch "bulkouttaxlot"
							if (bulkouttaxlot != null) {
								...
								if (bulkouttaxlot.OrionFirmId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[0] = String
											.valueOf(bulkouttaxlot.OrionFirmId);

								}

								if (bulkouttaxlot.ExternalId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[1] = String
											.valueOf(bulkouttaxlot.ExternalId);

								}

								if (bulkouttaxlot.AccountId != null) {

									row_tMysqlOutputBulkExec_1_tMOB[2] = bulkouttaxlot.AccountId;

								}

								if (bulkouttaxlot.Symbol != null) {

									row_tMysqlOutputBulkExec_1_tMOB[3] = bulkouttaxlot.Symbol;

								}

								if (bulkouttaxlot.AccountNumber != null) {

									row_tMysqlOutputBulkExec_1_tMOB[4] = bulkouttaxlot.AccountNumber;

								}

								if (bulkouttaxlot.DateAcquired != null) {

									row_tMysqlOutputBulkExec_1_tMOB[5] = FormatterUtils
											.format_Date(
													bulkouttaxlot.DateAcquired,
													"YYYY-MM-DD");

								}

								if (bulkouttaxlot.Quantity != null) {

									row_tMysqlOutputBulkExec_1_tMOB[6] = String
											.valueOf(bulkouttaxlot.Quantity);

								}

								if (bulkouttaxlot.CostAmount != null) {

									row_tMysqlOutputBulkExec_1_tMOB[7] = String
											.valueOf(bulkouttaxlot.CostAmount);

								}

								if (bulkouttaxlot.CostPerShare != null) {

									row_tMysqlOutputBulkExec_1_tMOB[8] = String
											.valueOf(bulkouttaxlot.CostPerShare);

								}

								if (bulkouttaxlot.Price != null) {

									row_tMysqlOutputBulkExec_1_tMOB[9] = String
											.valueOf(bulkouttaxlot.Price);

								}

								if (bulkouttaxlot.MarketValue != null) {

									row_tMysqlOutputBulkExec_1_tMOB[10] = String
											.valueOf(bulkouttaxlot.MarketValue);

								}

								if (bulkouttaxlot.PriceDate != null) {

									row_tMysqlOutputBulkExec_1_tMOB[11] = FormatterUtils
											.format_Date(
													bulkouttaxlot.PriceDate,
													"YYYY-MM-DD HH:mm:ss");

								}

								if (bulkouttaxlot.id != null) {

									row_tMysqlOutputBulkExec_1_tMOB[12] = String
											.valueOf(bulkouttaxlot.id);
						...
						if (row2 != null) {
							...
							{ // start of Var scope
								...
								input_rejected_count_tmp.fileName = "TaxlotFile  "
										+ (context.localpath + context.taxlotpath);
								input_rejected_count_tmp.component = "tinputfileDelimted";
								input_rejected_count_tmp.process = "taxlot Local to Staging";
					...
					if (!((Object) (context.localpath + context.taxlotpath) instanceof java.io.InputStream)) {
				...
				String tableName_tMysqlOutputBulkExec_1_tMBE = "taxLots_"
				...
				while (rsTable_tMysqlOutputBulkExec_1_tMBE.next()) {
					...
					if (table_tMysqlOutputBulkExec_1_tMBE
							.equalsIgnoreCase("taxLots_" + context.firmId + "_"
									+ context.date)) {
				...
				stmtCreate_tMysqlOutputBulkExec_1_tMBE
						.execute("CREATE TABLE `"
								+ tableName_tMysqlOutputBulkExec_1_tMBE
								+ "`(`OrionFirmId` INT(11)  ,`ExternalId` INT(11)  ,`AccountId` VARCHAR(255)  ,`Symbol` VARCHAR(255)  ,`AccountNumber` VARCHAR(255)  ,`DateAcquired` DATETIME ,`Quantity` FLOAT(11,6)  ,`CostAmount` FLOAT(11,5)  ,`CostPerShare` FLOAT(11,6)  ,`Price` FLOAT(12,4)  ,`MarketValue` FLOAT(11,5)  ,`PriceDate` DATETIME ,`id` INT(0)  )");
				...
				String file_tMysqlOutputBulkExec_1_tMBE = context.pathforlocal
						+ "taxLots_" + context.firmId + "_" + context.date
						+ ".tsv";
				...
				String bulkSQL_tMysqlOutputBulkExec_1_tMBE = "LOAD DATA LOCAL INFILE '"
						...
						+ lines_separator_tMysqlOutputBulkExec_1_tMBE
								.replaceAll("\\\\", "\\\\\\\\")
						+ "' (`OrionFirmId`,`ExternalId`,`AccountId`,`Symbol`,`AccountNumber`,`DateAcquired`,`Quantity`,`CostAmount`,`CostPerShare`,`Price`,`MarketValue`,`PriceDate`,`id`)";
	...
	public static void main(String[] args) {
		final StgTaxLotsImport StgTaxLotsImportClass = new StgTaxLotsImport();

		int exitCode = StgTaxLotsImportClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = StgTaxLotsImport.class
					.getClassLoader().getResourceAsStream(
							"reliazedgainloss/stgtaxlotsimport_0_1/contexts/"
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : StgTaxLotsImport");
		}
...
/************************************************************************************************
 * 197703 characters generated by Talend Open Studio for Data Integration on the
 * October 24, 2016 5:33:53 PM IST
 ************************************************************************************************/
