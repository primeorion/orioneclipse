...
MySQL - 5.6.10 : Database - qa_orionEclipseFirm_999
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`qa_orionEclipseFirm_999` /*!40100 DEFAULT CHARACTER SET latin1 */;

USE `qa_orionEclipseFirm_999`;
...
) ENGINE=InnoDB AUTO_INCREMENT=2490 DEFAULT CHARSET=latin1;
...
/*Table structure for table `dataLoadLog` */
...
CREATE TABLE `dataLoadLog` (
  ...
  `startExtractTime` datetime NOT NULL,
  `endExtractTime` datetime DEFAULT NULL,
  `startImportTime` datetime DEFAULT NULL,
  `endImportTime` datetime DEFAULT NULL,
  `startAnalyticsTime` datetime DEFAULT NULL,
  `endAnalyticsTime` datetime DEFAULT NULL,
...
/*Table structure for table `dataLoadLogDetail` */

DROP TABLE IF EXISTS `dataLoadLogDetail`;

CREATE TABLE `dataLoadLogDetail` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `dataLoadLogId` varchar(100) NOT NULL,
...
/*Table structure for table `databaseInfrastructure` */

DROP TABLE IF EXISTS `databaseInfrastructure`;

CREATE TABLE `databaseInfrastructure` (
  `connectionString` varchar(50) DEFAULT NULL,
  `currentDatabaseCount` int(11) DEFAULT NULL,
...
DROP TABLE IF EXISTS `fileCount`;

CREATE TABLE `fileCount` (
  `fileName` varchar(100) DEFAULT NULL,
  `firmId` varchar(50) DEFAULT NULL,
  `date` date DEFAULT NULL,
  `numberOfRowsInTsvFile` int(10) DEFAULT NULL,
  `numberOfRowsInsertedInStaging` int(10) DEFAULT NULL,
  `numberOfRowsInsertedInFirm` int(10) DEFAULT NULL,
  `numberOfRowsUpdatedInFirm` int(10) DEFAULT NULL,
  `numberOfRowsRejectInFirm` int(10) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `security` (
  ...
  CONSTRAINT `security_ibfk_2` FOREIGN KEY (`assetClassId`) REFERENCES `assetClass` (`id`),
  CONSTRAINT `security_ibfk_3` FOREIGN KEY (`assetSubClassId`) REFERENCES `assetSubClass` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE `statcatcher` (
  ...
  `jobOn` varchar(255) DEFAULT NULL,
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `deleteUser`(var_userId INT)
BEGIN
##-- Author: Ashutosh Verma
##-- Created On: 27 july,2016
##-- call deleteUser (1234)
##----- SP to delete userId from user table -----##
## -----  userId will be passed as a parameter.
##-----Sql query to delete userId from user table-----#
DELETE FROM userTeam WHERE userId = var_userId;
DELETE FROM userRole WHERE userId = var_userId;
DELETE FROM `user` WHERE userId = var_userId;
END */$$
DELIMITER ;
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `getDashboardSummary`(var_user_id INT)
BEGIN
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `getDashboardSummaryCustodians`(var_user_id INT)
BEGIN
##-- Author: Ashutosh Verma
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `getDashboardSummaryRoles`(var_user_id INT)
BEGIN
##-- Author: Ashutosh Verma
##-- Created On: 3rd August,2016
...
DECLARE var_roleType VARCHAR(20);
SELECT rt.roleType INTO var_roleType FROM `user` ur INNER JOIN role r ON ur.roleId = r.id INNER JOIN roleType rt ON r.roleType = rt.id WHERE ur.userId = var_user_id;
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `getDashboardSummaryTeams`(var_user_id INT)
BEGIN
##-- Author: Ashutosh Verma
##-- Created On: 3rd August,2016
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `getDashboardSummaryUsers`(var_user_id INT)
BEGIN
##-- Author: Ashutosh Verma
##-- Created On: 3rd August,2016
...
/*!50003 CREATE DEFINER=`oEAQA`@`%` PROCEDURE `updateUserStatus`(varTeamId INT)
BEGIN
##-- Author: Ashutosh Verma
