...
export class DonutChartService {
    ...
    public renderDonutChart(parent, chartData, colorInfo) {
        ...
        g.on("mousemove", function (d) {
           ...
           div.html((d.data["name"]) + '<br>' + (d.data["percent"]) + '%' + '<br>' + "$" + d3.format(",")(d.data["marketValue"]))
                 .style("font-weight", "bold");
            d3.select('.ghostPath' + d.data["name"]).style('display', 'inline');
        });

        g.on("mouseout", function (d) {
            div.style("display", "none");
            d3.select('.ghostPath' + d.data["name"]).style('display', 'none');
        });
